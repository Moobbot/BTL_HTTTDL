...
BEGIN
-- Find nearest node
EXECUTE 'SELECT id::integer FROM dongda_street_vertices_pgr
...
SELECT (route.geom)
FROM (SELECT geom
	FROM pgr_fromAtoB('dongda_street', 0, 0, -1, -1)
	ORDER BY seq) AS route;

--
SELECT name, ST_AsGeoJson(p.geom) from dongda_univercity_point as p
join dongda_univercity as u
on u.osm_id = p.osm_id
--
SELECT * FROM dongda_univercity AS u WHERE u.gid = 1;
